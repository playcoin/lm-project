...
class RnnWFWS(RnnWS):
	...
	def traintext(self, train_text, train_tags, test_text, test_tags, 
		...
		e_time = time.clock()
		print "RnnWSWF train over!! The total training time is %.2fm." % ((e_time - s_time) / 60.) 
...
class RnnWFWS2(RnnWS):
	...
	def reshape(self, in_data, in_data_f, in_data_f_2, l_data):
		s_in = in_data.reshape(self.batch_size, in_data.shape[0] / self.batch_size).T
		s_in_f = in_data_f.reshape(self.batch_size, in_data.shape[0] / self.batch_size).T
		s_in_f_2 = in_data_f_2.reshape(self.batch_size, in_data.shape[0] / self.batch_size).T
		s_l = l_data.reshape(self.batch_size,  l_data.shape[0] / self.batch_size).T
		return theano.shared(s_in, borrow=True), theano.shared(s_in_f, borrow=True), theano.shared(s_in_f_2, borrow=True), theano.shared(s_l, borrow=True)
	...
	def traintext(self, train_text, train_tags, test_text, test_tags, 
		...
		re_mat_in, re_mat_in_f, re_mat_in_f_2, re_label = self.reshape(mat_in_total[:data_size], mat_in_f_total[:data_size], mat_in_f_2_total[:data_size], label_total[:data_size])
		self.rnn.build_tbptt(re_mat_in, re_mat_in_f, re_mat_in_f_2, re_label, self.truncate_step, self.train_emb, l2_reg=self.l2_reg)
		print "Compile Truncate-BPTT Algorithm complete!"
